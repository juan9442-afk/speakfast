...
export interface OfertaProps {
  ...
  /** Línea de confianza bajo AMBOS CTA de plan ("15 días de garantía · cancelas cuando
   * quieras") — repite la garantía cerca del botón de compra real, no solo en la
   * nota del stack (52 §2: el precio nunca aparece sin su ancla de riesgo cerca). */
  ...
  return (
    <SectionShell id={id} elevacion="base" ariaLabel="Planes y precios">
      <motion.div variants={contenedor} initial="hidden" whileInView="visible" viewport={VIEWPORT_ONCE}>
        ...
        <div className="mx-auto mt-12 grid max-w-[880px] grid-cols-1 items-start gap-6 md:grid-cols-2">
        ...
        {/* Ancla de riesgo repetida cerca del CTA real (52 §2) — no solo en la nota del stack */}
        {garantiaTexto && (
          <motion.p
            variants={item}
            className="mt-5 flex items-center justify-center gap-1.5 text-center text-[13px] text-[var(--text-secondary)]"
          >
