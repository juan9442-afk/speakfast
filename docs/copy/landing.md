...
## AJUSTES POR DECISIÓN DE PRODUCTO 4bis (2026-09-05 — "entrevista con voz + chuleta")

El usuario decidió que el mecanismo pasa de "1 pregunta suelta" a "entrevista completa: una voz de IA hace 4-6 preguntas seguidas" + cada entrevista deja una "chuleta" (frases anti-bloqueo + upgrades de vocabulario). El copy se alineó:
1. **Hero subtítulo**: "tu entrevista real, con cronómetro" → "una entrevista completa, con voz y cronómetro".
2. **Solución (pasos)**: paso 2 "Respondes bajo cronómetro / 60 segundos, en voz alta" → "Una voz te entrevista / 4 a 6 preguntas seguidas, con cronómetro"; paso 3 "Recibes tu puntaje / …tu respuesta reescrita" → "Sales con tu chuleta / …tus frases anti-bloqueo, listas para usar". Big Idea: "te la pone hoy" → "te entrevista hoy".
3. **Carrusel (AppPorDentro)**: labels de simulación y resultado → "Una voz te entrevista, pregunta tras pregunta" / "Tu puntaje + tu chuleta de frases para el día real".
4. **Oferta**: se agregó al value stack la línea "Kit de frases anti-bloqueo + tu chuleta" ($29, total tachado $273, "Ahorras $228 al año"); los `features` de ambos planes ahora listan "Entrevistas completas… todos los días", "Kit de frases anti-bloqueo + tu chuleta, cada vez", "Tu diccionario de 'sonar profesional' que crece contigo".
5. **FAQ #3** (¿una IA puede simular la presión?): respuesta ahora dice "Una voz te hace pregunta tras pregunta con cronómetro…".
6. **CTA Final** (future pacing): "respondes bajo cronómetro — y en minutos sabes qué corregir" → "Una voz te entrevista pregunta tras pregunta — y sales con tu chuleta lista para el día real".
7. **Mockups** `public/mockups/simulacion.svg` y `resultado.svg` reescritos: la simulación muestra "Sarah · Recursos Humanos · Pregunta 3 de 5" + "Siguiente pregunta"; el resultado muestra "Pausa por traducción" + una card "TU CHULETA · 3 FRASES NUEVAS" + botón "Abrir mi chuleta". (Se reemplazan por capturas reales en la Sesión 5.)
Nada de esto contradice FICHA-AVATAR: la voz + la chuleta atacan directamente el dolor #1 ("se me pone la mente en blanco") y el deseo de "sonar caro/profesional".
