...
- Fondo: ~~`#FAF6EC`~~ → **`#F3ECDA`** (crema cálido, un punto más profundo — ver "Reapertura 2026-09-04") · Superficie: `#FFFDF7` · Hundido: ~~`#F2EBDA`~~ → **`#E7DBBE`** · Texto 1º/2º: `#241F1A` / ~~`#7C7568`~~ → **`#665F52`** · Texto 3º: ~~`#665F52`~~ → **`#4A4438`** (los dos se oscurecieron un escalón el 2026-09-05: el fondo más profundo bajó el contraste de texto-secundario a ~3.87:1, bajo AA — medido por revisor-visual en Sesión 4)
...
## Logo (2026-09-05 — pedido del usuario)
Isotipo: **cuadrado redondeado verde** (`--accent`, radio grande al estilo del brand kit) con una **onda de voz de 4 barras que ASCIENDE** (hablar + mejorar bajo presión), con **leve inclinación hacia adelante** (velocidad = "Fast"). Las 3 primeras barras en `--surface` (crema), la 4ª (la más alta, el pico que entrenas) en la 2ª nota cálida. Wordmark "SpeakFast" en **Fraunces** (display). Archivos: `app/icon.svg` (favicon, con hex literales), `components/brand.tsx` (`<Isotipo>` y `<Logo>` con tokens). Integrado en: header del Hero, footer legal, header del funnel (onboarding/paywall/login), header de la app interna. Traza a FICHA-ARTE: arquetipo Héroe (superación), verde "listo/aprobado", 2ª nota terracota como acento puntual, forma redondeada del kit.
Demo animada del Hero: `components/landing/HeroDemo.tsx` — un "GIF" hecho con SVG + motion (sin archivo de video, pesa nada, funciona en Android gama media de LATAM), loop de ~7s (pregunta → grabando con cronómetro → resultado + chuleta), reduced-motion se queda en el frame del resultado. Reemplaza el mockup estático `resultado.svg` en el Hero (el carrusel sigue con los SVG hasta la Sesión 7).
